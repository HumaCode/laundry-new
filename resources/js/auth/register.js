document.addEventListener('DOMContentLoaded', () => {
    ...
    function startLockoutUI(totalSeconds) {
        ...
        btnText.text('Pendaftaran Terkunci 🔒');

        const minutes = Math.ceil(totalSeconds / 60);
        errorText.html(
            `🔒 Terlalu banyak percobaan pendaftaran.<br>` +
        ...
        if (typeof window.showToast === 'function') {
            window.showToast(
                `Pendaftaran diblokir ${minutes} menit karena terlalu banyak percobaan.`,
                'error',
                '🔒 Akses Dibatasi'
            );
        }
        ...
        lockoutInterval = setInterval(() => {
            ...
            if (remaining <= 0) {
                ...
                errorText.html('');
                if (typeof window.showToast === 'function') {
                    window.showToast('Anda dapat mencoba melakukan pendaftaran kembali.', 'success', '✅ Blokir Dicabut');
                }
            }
        }, 1000);
    }
    ...
    registerForm.addEventListener('submit', function (e) {
        ...
        $.ajax({
            ...
            success(response) {
                ...
                if (typeof window.showToast === 'function') {
                    window.showToast(
                        `Pendaftaran berhasil! Selamat bergabung, ${response.user.name}! 🚀`,
                        'success',
                        'Registrasi Sukses'
                    );
                }
            ...
            error(xhr) {
                ...
                if (status === 200) {
                    if (typeof window.showToast === 'function') {
                        window.showToast('Pendaftaran berhasil! Mengalihkan...', 'success');
                    }
                    setTimeout(() => {
                ...
                // ── Generic fallback ──
                console.error('AJAX Register Error:', status, xhr.statusText, xhr.responseText);
